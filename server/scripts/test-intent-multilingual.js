...
const testCases = [
  // ==================== ENGLISH TESTS ====================
  { language: 'English', type: 'MEMORY_STORE', prompt: 'My name is John Smith' },
  { language: 'English', type: 'MEMORY_STORE', prompt: 'I am 30 years old' },
  { language: 'English', type: 'MEMORY_STORE', prompt: 'My favorite color is blue' },
  { language: 'English', type: 'MEMORY_STORE', prompt: 'I work as a software engineer' },
  ...
  { language: 'English', type: 'SEARCH_WEB', prompt: 'What is the weather in London today?' },
  { language: 'English', type: 'SEARCH_WEB', prompt: 'Tell me about recent AI breakthroughs' },
  { language: 'English', type: 'CHAT', prompt: 'Hello, how are you?' },

  // ==================== GERMAN TESTS ====================
  { language: 'German', type: 'MEMORY_STORE', prompt: 'Mein Name ist Anna Müller' },
  { language: 'German', type: 'MEMORY_STORE', prompt: 'Ich bin 28 Jahre alt' },
  { language: 'German', type: 'MEMORY_STORE', prompt: 'Meine liebste Farbe ist Grün' },
  { language: 'German', type: 'MEMORY_STORE', prompt: 'Ich arbeite als Ärztin' },
  ...
  { language: 'German', type: 'SEARCH_WEB', prompt: 'Wie ist das Wetter in Berlin heute?' },
  { language: 'German', type: 'SEARCH_WEB', prompt: 'Erzähl mir von den neuesten Technologie-News' },
  { language: 'German', type: 'CHAT', prompt: 'Guten Tag, wie geht es dir?' },

  // ==================== ITALIAN TESTS ====================
  { language: 'Italian', type: 'MEMORY_STORE', prompt: 'Mi chiamo Marco Rossi' },
  { language: 'Italian', type: 'MEMORY_STORE', prompt: 'Ho 32 anni' },
  { language: 'Italian', type: 'MEMORY_STORE', prompt: 'Il mio colore preferito è rosso' },
  { language: 'Italian', type: 'MEMORY_STORE', prompt: 'Lavoro come insegnante' },
  ...
  { language: 'Italian', type: 'SEARCH_WEB', prompt: 'Quale è il meteo a Roma oggi?' },
  { language: 'Italian', type: 'SEARCH_WEB', prompt: 'Dimmi le ultime notizie sulla tecnologia' },
...
async function sendTestPrompt(testCase, index) {
  return new Promise(async (resolve) => {
    try {
      const result = await request('/api/chat/stream', 'POST', { message: testCase.prompt });
      resolve({
        index,
        testCase,
        success: result.ok,
...
async function runTests() {
  ...
  console.log('\n╔════════════════════════════════════════════════════════════════╗');
  console.log('║         MULTI-LANGUAGE INTENT CLASSIFICATION TEST            ║');
  console.log('║                  (English, German, Italian)                  ║');
  console.log('╚════════════════════════════════════════════════════════════════╝\n');

  console.log(`📋 Total test cases: ${testCases.length}\n`);
  console.log('💡 Instructions:');
  console.log('   1. Watch the server console for [intent-raw] and [intent-classified] logs');
  console.log('   2. Verify each classification matches the expected type in the list below');
  console.log('   3. Check that memory is properly stored/retrieved across languages\n');
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    console.log(`${status} [${i + 1}/${testCases.length}] ${language} | ${type} | "${testCase.prompt.substring(0, 40)}${testCase.prompt.length > 40 ? '...' : ''}"`);
  ...
  console.log('3. Check [memory-query] logs to see if memory retrieval works across languages');
  console.log('4. Note any patterns in failures (if any)\n');

  if (failureCount > 0) {
    console.log('⚠️  Some tests failed. Check network connectivity and server status.\n');
  } else {
    console.log('✨ All prompts sent successfully! Check server logs for classifications.\n');
  }
}
