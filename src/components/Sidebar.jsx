  ...
  return (
    <aside ref={sidebarRef} className={`sidebar ${isOpen ? 'open' : 'collapsed'}`}>
      ...
      <div className="sidebar-top">
        <button className="sidebar-icon-btn" onClick={handleNewChat} title={t('sidebar.newChat')}>
        ...
        <div className="sidebar-chats-section">
          ...
          <div className="chats-list">
            {filteredChats.length === 0 ? (
              <p className="no-chats">{searchQuery ? t('sidebar.noChatsFound') : t('sidebar.noChats')}</p>
            ) : (
              filteredChats.map((chat) => (
                ...
                >
                  <span className="chat-name">{chat.preview || t('sidebar.chatLabel', { id: chat.filename.slice(-6) })}</span>
                  <button
