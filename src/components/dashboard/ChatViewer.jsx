...
export default function ChatViewer({
  ...
  useEffect(() => {
    ...
    authFetch(`/api/admin/chats/${chat.id}/messages`)
      ...
      .catch(() => {
        setError(t("dashboard.viewer.loadError"));
        setLoading(false);
      });
  }, [chat]);
  ...
  return (
    <div className="db-modal-backdrop" onClick={onClose}>
      <div
        className="db-modal db-modal-wide"
      ...
      >
        <div className="db-modal-header">
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div className="db-chat-meta-row">
              {chat.username && (
                <button
                  className="db-meta-user-btn"
                  onClick={() =>
                    onUserClick({ username: chat.username, id: chat.user_id })
                  }
                >
          ...
          >
            <button
              ...
              title={t("dashboard.viewer.deleteChat")}
            >
              {" "}
              <svg
                width="16"
        ...
        <div className="db-chat-viewer-body">
          {loading && (
            <div className="db-chat-loading">
              {Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className={`db-chat-skel-row ${i % 2 === 0 ? "right" : "left"}`}
                >
                  <div
          ...
          {error && (
            <div className="db-form-error" style={{ margin: 16 }}>
              {error}
            </div>
          )}
          ...
          {!loading &&
            messages.map((m, i) => (
              <div
                key={i}
                className={`db-cv-message ${m.role === "user" ? "user" : "ai"}`}
              >
