...
export default function ActivityChart({ users, chats, loading }) {
  ...
  if (loading)
    return (
      <div className="db-skeleton" style={{ height: 400, borderRadius: 12 }} />
    );
  ...
  return (
    <div className="db-activity-root">
      <div className="db-chart-card">
        <div className="db-panel-header">
          <span className="db-panel-title">
            Neue Registrierungen & Chats (30 Tage)
          </span>
        </div>
          ...
          >
            <defs>
              ...
              <linearGradient id="gradChats" x1="0" y1="0" x2="0" y2="1">
                <stop
                  offset="5%"
                  stopColor="var(--db-accent-violet)"
                  stopOpacity={0.3}
                />
                <stop
                  offset="95%"
      ...
      <div className="db-chart-card">
        <div className="db-panel-header">
          <span className="db-panel-title">
            Tägliche Chat-Erstellung (Balken)
          </span>
        </div>
        <ResponsiveContainer width="100%" height={260}>
          <BarChart
          ...
          >
            ...
            <Bar
              dataKey="Chats"
              fill="var(--db-accent-violet)"
              radius={[4, 4, 0, 0]}
            />
            <Bar
              dataKey="Benutzer"
              fill="var(--db-accent-teal)"
              radius={[4, 4, 0, 0]}
            />
          </BarChart>
        </ResponsiveContainer>
      ...
      <div className="db-activity-totals">
        ...
        <div className="db-activity-total-card">
          <span className="db-at-num">
            {chats.length
              ? Math.round(
                  chats.reduce((s, c) => s + (c.message_count || 0), 0) /
                    chats.length,
                )
              : 0}
          </span>
